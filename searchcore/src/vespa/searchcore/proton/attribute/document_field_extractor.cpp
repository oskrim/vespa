...
=======
DocumentFieldExtractor::extractSpecificKeyValueFromMap(const FieldPath &fieldPath)
{
    const FieldValue *mapFieldValue = getCachedFieldValue(fieldPath[0]);
    if (mapFieldValue && mapFieldValue->isA(FieldValue::Type::MAP)) {
        const auto *outerMap = static_cast<const MapFieldValue *>(mapFieldValue);
        const FieldPathEntry &keyEntry = fieldPath[1];
        const FieldValue &lookupKey = keyEntry.getLookupKey();
        
        for (const auto &mapElem : *outerMap) {
            if (mapElem.first->compare(lookupKey) == 0) {
                return std::unique_ptr<FieldValue>(mapElem.second->clone());
            }
        }
    }
>>>>>>> Stashed changes
...
=======
            return extractSpecificKeyValueFromMap(fieldPath);
>>>>>>> Stashed changes
        } else {
